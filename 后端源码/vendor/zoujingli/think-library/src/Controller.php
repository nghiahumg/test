...
// +----------------------------------------------------------------------
// | 版权所有 2018~2022 纸飞机：@kuyu886 [ http://www.kuyu.pro ]
// +----------------------------------------------------------------------
...
abstract class Controller extends \stdClass
{
    ...
    protected function checkToken(){
        if(!$this->request->param('token'))
        {
            ...
            $returnData = array(
                ...
                "tr_tr" =>$tips->value('tr_tr'),
                "es_es" =>$tips->value('es_es'),
            );
            $this->error($returnData,'',403);
        }
        try{
            $result = JWT::decode($this->request->param('token'), '6nUS[@.1-TH^mL{dW1N>ZAfqJ:q&4l+jtXoCy@h0PI~', array('HS256'));//进行解密算法
    ...
    public function getToken($user){
        $token = array(
            ...
            'exp' => time() + 3600*7,           //过期时间
            'data' => array(
                'id'    =>  $user['id'],
                'phone'  =>  $user['phone'],
            )
        );
        $jwt = JWT::encode($token, '6nUS[@.1-TH^mL{dW1N>ZAfqJ:q&4l+jtXoCy@h0PI~');
        return $jwt;
    }
    ...
    private function decryptToken($jwtToken){
        try{
            $result = JWT::decode($jwtToken, '6nUS[@.1-TH^mL{dW1N>ZAfqJ:q&4l+jtXoCy@h0PI~', array('HS256'));//进行解密算法
