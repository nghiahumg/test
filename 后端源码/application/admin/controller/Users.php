...
class Users extends Controller
{
    ...
    public function index()
    {
        ...
        $query->join('lc_user_member m','u.member=m.id')->where($where)->equal('u.real_name#u_auth,u.clock#u_clock,u.member#u_member')->like('u.phone#u_phone,u.agent#u_agent,u.ip#u_ip')->dateBetween('u.time#u_time')->order('u.id desc')->page();
    }
        ...
        if ($this->request->isPost()) {
            ...
            if(isset($vo['id'])){
                ...
                if($money&&$money != $vo['money']){
                    ...
                    model('admin/Users')->addFinance($vo['id'],abs($handle_money),$type,'系统操作','系統操作','System Operation','การทำงานของระบบ','Vận hành hệ thống','システム操作','시스템 운영','Operasi sistem','Sistem çalışması','Operación del sistema');
                }
            }else{
